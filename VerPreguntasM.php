<?php
	...
	echo '<center><table border=1> <tr><th> Seleccion </th> <th> Numero Pregunta </th> <th> Autor </th> <th> Dificultad </th> <th> Pregunta </th> </tr></center>';
	
	while($row = mysqli_fetch_array($resultado))
	{
	//echo '<tr> <td>''<input type="radio" name="'.$row['Num_preg'].'"'.'</td><td>'.$row['Email'].'</td><td>'.$row['Dificultad'].'</td><td>'.$row['Pregunta'].'</td></tr>';
	echo '<tr> <td> <input type="radio" id="preguntas" name="preguntas" value="'.$row['Num_preg'].'" onClick="javascript:verPreguntaD(this.value)"><td>'.$row['Num_preg'].'</td><td>'.$row['Email'].'</td><td>'.$row['Dificultad'].'</td><td>'.$row['Pregunta'].'</td></td></tr> ';
	}
